...
func main() {
	...
	if err != nil {
		log.Fatalf("[%s] error leyendo catalogo: %v", *discoteca, err)
	}
	...
	if len(misPlantillas) == 0 {
		log.Fatalf("[%s] no se encontraron eventos para esta discoteca en %s", *discoteca, *catalogo)
	}
	log.Printf("[%s] encontradas %d plantillas", *discoteca, len(misPlantillas))

	connBroker, err := grpc.NewClient(*dirBroker, grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
		log.Fatalf("[%s] no pude conectar al broker: %v", *discoteca, err)
	}
	...
	for {
		...
		cancel()
		if err == nil && ack.Ok {
			log.Printf("[%s] registrado en broker exitosamente", *discoteca)
			break
		}
		log.Printf("[%s] esperando al broker... (%v)", *discoteca, err)
	...
	for {
		...
		if reenvio {
			...
			log.Printf("[%s] REENVIO (idempotencia) evento %s", *discoteca, evento.EventoId)
		}
		...
		if invalido {
			...
			log.Printf("[%s] evento INVALIDO (categoria=%s) para probar validacion", *discoteca, evento.Categoria)
		}
		...
		cancel()
		if err != nil {
			log.Printf("[%s] error al publicar evento: %v", *discoteca, err)
		} else if resp.Aceptado {
			log.Printf("[%s] evento ACEPTADO: %s", *discoteca, evento.NombreEvento)
		} else {
			log.Printf("[%s] evento RECHAZADO: %s motivo=%s", *discoteca, evento.NombreEvento, resp.Motivo)
		}
